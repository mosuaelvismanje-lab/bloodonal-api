...
# =========================================================
def create_mock_payment_response(
    success: bool = True,
    ref: str = "TAXI-REF-999"
):
...
async def test_pay_taxi_success(client, monkeypatch):
    ...
    mock_repo = AsyncMock()
    mock_repo.try_consume_free_usage.return_value = True
    ...
    response = await client.post(
        ...
        json={
            "phone": "670000000",
            "taxi_driver_id": "DRV-123",
            "ride_distance_km": 5.0
        }
    )
...
async def test_get_remaining_taxi_rides(client, monkeypatch):
    ...
    mock_repo = AsyncMock()
    mock_repo.count_uses.return_value = 2
    ...
    mock_repo.count_uses.assert_called_once()
...
async def test_pay_taxi_exceeds_limit(client, monkeypatch):
    ...
    mock_repo = AsyncMock()
    mock_repo.try_consume_free_usage.return_value = False

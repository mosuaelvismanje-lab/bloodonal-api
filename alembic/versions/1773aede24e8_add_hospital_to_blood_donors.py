"""Add hospital to blood_donors

Revision ID: 1773aede24e8
Revises: 
Create Date: 2025-06-27 03:06:24.665645
...
def upgrade() -> None:
    ...
    # --- 1. Tables to be permanently dropped (User confirmed) ---
    # Dropping symptom_reports table and its index (safely, if_exists=True)
    op.drop_index(op.f('ix_symptom_reports_id'), table_name='symptom_reports', if_exists=True)
    op.drop_table('symptom_reports', if_exists=True)

    # Dropping outbreaks table and its index (safely, if_exists=True)
    op.drop_index(op.f('ix_outbreaks_id'), table_name='outbreaks', if_exists=True)
    op.drop_table('outbreaks', if_exists=True)

    # Dropping vaccination_campaigns table and its index (safely, if_exists=True)
    op.drop_index(op.f('ix_vaccination_campaigns_id'), table_name='vaccination_campaigns', if_exists=True)
    op.drop_table('vaccination_campaigns', if_exists=True)

    # --- 2. Primary Action of this Migration File ---
    # The name of this migration file implies adding a 'hospital' column to 'blood_donors'.
    # This step is required to complete the migration successfully.
    # NOTE: You must use the correct data type (sa.Integer or sa.UUID) and Foreign Key reference
    # based on your 'hospital' or 'healthcare_providers' table structure.
    # I am using 'sa.Integer' and assuming the foreign key refers to 'healthcare_providers.id' for demonstration.

    import sqlalchemy as sa
    op.add_column('blood_donors',
                  sa.Column('hospital_id', sa.Integer(), sa.ForeignKey('healthcare_providers.id'), nullable=True))
